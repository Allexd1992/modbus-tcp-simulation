...
- Tags: e.g. **`2.1.1`**
- **CI** (`.github/workflows/ci.yml`): on non-PR pushes, the image is built and pushed **only to Docker Hub** (`:sha-<short>`, `:test` on branches; `:latest` and `:<version>` on semver tags). **Platforms:** branch/test builds **`linux/amd64` only**; **release tags** add **`linux/arm64`** (multi-arch manifest). Set secrets **`DOCKERHUB_USERNAME`** and **`DOCKERHUB_TOKEN`** ([Docker Hub access token](https://hub.docker.com/settings/security)).
